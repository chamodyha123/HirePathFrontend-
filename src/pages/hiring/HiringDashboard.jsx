// src/pages/hiring/HiringDashboard.jsx
import React, { useEffect, useMemo, useState } from 'react';
import HiringSidebar from './HiringSidebar';
import applicationApi, { STATUS_VALUE, statusLabel, normalizeApplication, normalizeCollection } from "../../api/applicationApi";
      ...
      <main style={{ flex: 1, padding: "38px 42px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,minmax(0,1fr))", gap: 20, marginBottom: 28 }}>
          {[
            ...
            ["INTERVIEWS SCHEDULED", stats.scheduled, "📅"],
            ["AWAITING DECISION", stats.awaiting, "🧾"],
            ["HIRED", stats.hired, "✅"]
            ...
            <div key={label} style={{ background: "white", border: "1px solid #e5e7eb", borderRadius: 16, padding: 24, display: "flex", justifyContent: "space-between" }}>
              <div>
                ...
                <div style={{ fontSize: 34, fontWeight: 800, marginTop: 12 }}>{value}</div>
              </div>
        ...
        <section style={{ background: "white", border: "1px solid #e5e7eb", borderRadius: 16, padding: 26 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 18 }}>
            <div>
              <h2 style={{ margin: 0, fontSize: 20 }}>Candidate & Interview Pipeline</h2>
              <p style={{ margin: "6px 0 0", color: "#94a3b8" }}>Live shortlisted applications and interview records.</p>
            </div>
          ...
          {loading ? (
            ...
            <div style={{ padding: "45px 0", textAlign: "center", color: "#94a3b8" }}>No shortlisted candidates or interviews right now.</div>
          ) : (
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ background: "#f8fafc", textAlign: "left" }}>
                    {["Candidate", "Job", "Status", "Interview", "Match", "Actions"].map((h) => (
                      <th key={h} style={{ padding: 13 }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {apps.map((app) => {
                    ...
                    return (
                      <tr key={app.id} style={{ borderTop: "1px solid #e5e7eb" }}>
                        <td style={{ padding: 14 }}>
                          <b>{nameOf(app)}</b>
                          <div style={{ color: "#94a3b8", fontSize: 12 }}>{emailOf(app)}</div>
                        </td>
                        <td style={{ padding: 14 }}>{jobOf(app)}</td>
                        <td style={{ padding: 14 }}>
                          <span style={{ background: "#eef2ff", color: "#4338ca", padding: "6px 9px", borderRadius: 20, fontSize: 12, fontWeight: 700 }}>
                        ...
                        <td style={{ padding: 14 }}>
                          {interview ? (
                            <>
                              ...
                              <br />
                              <small>{interview.meetingLink || interview.location || interview.interviewType}</small>
                            </>
                          ) : "Not scheduled"}
                        </td>
                        ...
                        <td style={{ padding: 14 }}>
                          <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
                            {app.status === STATUS_VALUE.Shortlisted && !activeInterview && (
                              <button
                                disabled={busy === app.id}
                                onClick={() => run(app.id, () => schedule(app))}
                                style={{ ...btn, background: "#dbeafe", color: "#1d4ed8" }}
                              >
                                Schedule
                              </button>
                            )}
                            ...
                            {activeInterview && (
                              <>
                                <button
                                  disabled={busy === app.id}
                                  onClick={() => run(app.id, () => reschedule(interview))}
                                  style={{ ...btn, background: "#e0f2fe", color: "#0369a1" }}
                                >
                                  Reschedule
                                </button>
                                <button
                                  ...
                                  style={{ ...btn, background: "#fee2e2", color: "#b91c1c" }}
                                >
                                  Cancel
                                </button>
                                <button
                                  disabled={busy === app.id}
                                  onClick={() => run(app.id, () => submitFeedback(app, interview))}
                            ...
                            {[STATUS_VALUE.Interviewed, STATUS_VALUE.Offered].includes(app.status) && (
                              <button
                                disabled={busy === app.id}
                                onClick={() => run(app.id, () => evaluate(app))}
                                style={{ ...btn, background: "#f3e8ff", color: "#7e22ce" }}
                              >
                                Evaluate
                              </button>
                            )}
                            ...
                            {app.status === STATUS_VALUE.Interviewed && (
                              <button
                                ...
                                style={{ ...btn, background: "#fef3c7", color: "#92400e" }}
                              >
                                Make Offer
                              </button>
                            )}
